    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name sonnt-mysql --rm --network mysql -v sonnt-mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_LOGIN_PSW} -e MYSQL_DATABASE=db_example  -d mysql:8.0 "
                sh 'sleep 20'
        ...
        stage('Deploy Spring Boot to DEV') {
            steps {
                ...
                sh 'docker container run -d --rm --name ngotheson-springboot -p 8081:8080 --network dev ngotheson/springboot'
            }
        }
